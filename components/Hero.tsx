...
export const Hero: React.FC<HeroProps> = ({ onStart, onImpact }) => {
  return (
    <div className="relative bg-[#0c0c0c] min-h-screen flex items-center overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 pt-32 pb-24 lg:pt-48">
        <div className="max-w-4xl animate-app-reveal">
          <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-amber-600/10 border border-amber-600/20 text-amber-500 text-[10px] font-black uppercase tracking-[0.3em] mb-10 shadow-2xl">
            <Sparkles className="w-3.5 h-3.5" /> Mysore Heritage Discovery
          </div>
          
          <h1 className="text-5xl md:text-8xl lg:text-9xl font-black font-serif mb-8 leading-[0.95] tracking-tight text-white">
            Beyond the <br/>
            <span className="text-amber-500 italic">Palace Walls.</span>
          </h1>
          
          <p className="text-lg md:text-2xl text-stone-300 mb-14 max-w-2xl leading-relaxed font-light">
          ...
          <div className="flex flex-col sm:flex-row gap-5 mb-28">
            <button 
              onClick={onStart}
              className="h-16 md:h-20 bg-amber-600 hover:bg-amber-500 text-white px-10 md:px-14 rounded-2xl md:rounded-[2.5rem] font-black text-[11px] md:text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-4 transition-all shadow-2xl shadow-amber-900/40 active:scale-95"
            >
              Start Exploring <ArrowRight className="w-5 h-5 md:w-6 md:h-6" />
            </button>
            <button 
              onClick={onImpact}
              className="h-16 md:h-20 bg-white/5 hover:bg-white/10 text-stone-100 border border-white/10 px-10 md:px-14 rounded-2xl md:rounded-[2.5rem] font-black text-[11px] md:text-xs uppercase tracking-[0.2em] backdrop-blur-xl transition-all active:scale-95"
            >
              Our Impact
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 animate-app-reveal" style={{ animationDelay: '0.2s' }}>
          {[
            ...
            <div key={i} className="glass-card p-10 rounded-[2.5rem] flex flex-col gap-6 card-lift">
              ...
              <div>
                <h4 className="font-bold text-white mb-3 text-xl font-serif">{item.title}</h4>
