...
import { MapPin, Star, Search, Loader2, X, Navigation, Sparkles, ArrowRight, ExternalLink, Globe } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 sm:mb-16 animate-app-reveal">
          ...
          <p className="text-stone-500 max-w-2xl mx-auto text-base sm:text-xl font-light leading-relaxed">
            Avoid the commercial traps. Find stories that define Mysuru's heartbeat, curated by real-time heritage data.
          </p>
        </div>
          ...
          <form onSubmit={handleSearch} className="relative p-1 rounded-[2.5rem] bg-gradient-to-r from-stone-900 via-amber-900/20 to-stone-900 border border-white/5 shadow-[0_30px_60px_rgba(0,0,0,0.6)]">
            <div className="flex items-center bg-[#0c0c0c] rounded-[2.3rem] px-4 md:px-6 py-2 md:py-3 overflow-hidden relative">
              ...
              <input 
                ...
                placeholder="Search: Try 'Rosewood inlay' or 'Silent lakes'..."
          ...
          {aiResults && (
            <div className="mt-8 bg-[#141414] border border-white/10 rounded-[2.5rem] p-8 sm:p-12 animate-app-reveal shadow-2xl relative overflow-hidden">
               ...
               <div className="flex items-center gap-2 text-[10px] font-black text-amber-500 uppercase tracking-widest mb-6">
                 <Sparkles className="w-4 h-4" /> AI Heritage Search Result
               </div>
               ...
               {/* Grounding Sources (Web & Maps) */}
               {aiResults.chunks && aiResults.chunks.length > 0 && (
                 <div className="space-y-4 pt-8 border-t border-white/5">
                   <p className="text-[9px] font-black text-stone-600 uppercase tracking-widest">Verified Sources & Maps</p>
                       ...
                       if (chunk.web) {
                         return (
                           <a 
                             ...
                             className="flex items-center gap-3 px-4 py-2.5 bg-stone-900 border border-white/5 rounded-xl text-[10px] font-bold text-stone-400 hover:text-white hover:border-amber-600/30 transition-all"
                           >
                             <Globe className="w-3.5 h-3.5 text-blue-500" /> {chunk.web.title || 'View Source'} <ExternalLink className="w-3 h-3" />
                           </a>
                         );
                       }
                       if (chunk.maps) {
                         return (
                           <a 
                             ...
                             className="flex items-center gap-3 px-4 py-2.5 bg-stone-900 border border-white/5 rounded-xl text-[10px] font-bold text-stone-400 hover:text-white hover:border-amber-600/30 transition-all"
                           >
                             <MapPin className="w-3.5 h-3.5 text-amber-600" /> {chunk.maps.title || 'Map Location'} <ExternalLink className="w-3 h-3" />
                           </a>
                         );
                       }
                       return null;
